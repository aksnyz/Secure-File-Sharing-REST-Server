...
// Request structure for sharing a file
#[derive(Deserialize)]
...
pub async fn list_files(
    ...
    let files_result = sqlx::query_as::<_, FileRecord>(
...
pub async fn download_file(
    ...
    let has_permission = sqlx::query("SELECT 1 FROM file_permissions WHERE file_id = ? AND user_id = ?")
        .bind(file_id)
        .bind(user_id)
        .fetch_optional(&pool)
        .await
    ...
    if !is_owner && !is_public && !has_permission {
        return (StatusCode::FORBIDDEN, "Access denied. File is private.").into_response().into_response();
    }
...
pub async fn share_file(
    ...
    //authenticate the requester (current user)
    let auth_header = headers.get(header::AUTHORIZATION)
        .and_then(|h| h.to_str().ok())
        .and_then(|h| h.strip_prefix("Bearer "));
    ...
    //get Requester ID (Owner)
    let owner = sqlx::query_as::<_, User>("SELECT * FROM users WHERE username = ?")
        .bind(&username)
        .fetch_optional(&pool)
        .await
        .unwrap_or(None);
    ...
    //verify that the requester actually owns the file
    let file: Option<FileRecord> = sqlx::query_as("SELECT * FROM files WHERE id = ? AND owner_id = ?")
    ...
    if file.is_none() {
        return (StatusCode::FORBIDDEN, "You are not the owner of this file").into_response();
    }

    //find the Target User (who we want to share with)
    ...
    let target_user_id = match target_user {
        Some(u) => u.id.unwrap(),
        None => return (StatusCode::NOT_FOUND, "Target user not found").into_response(),
    };

    //insert permission into DB
    let result = sqlx::query("INSERT OR IGNORE INTO file_permissions (file_id, user_id) VALUES (?, ?)")
        .bind(payload.file_id)
        .bind(target_user_id)
        .execute(&pool)
        .await;
    ...
    match result {
        Ok(_) => (StatusCode::OK, "File shared successfully").into_response(),
